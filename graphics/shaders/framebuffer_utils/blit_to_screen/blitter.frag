...
void main() {
    ...
    vec4 opaque = texture(color, tex_coord);

    float inv_accum_w = (accum.a > 1e-5) ? 1.0 / accum.a : 0.0;
    vec3 transparent_color = accum.rgb * inv_accum_w;
    float transparent_alpha = 1.0 - reveal;

    // -- final composite

    vec3 composite_rgb = transparent_color * transparent_alpha + opaque.rgb * (1.0 - transparent_alpha);
    
    // Final output with combined alpha
    frag_color = vec4(composite_rgb, max(transparent_alpha, opaque.a));
}